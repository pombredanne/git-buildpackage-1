...
from gbp.rpm import (RpmPkgPolicy, SpecFile,
                     NoSpecError, guess_spec, guess_spec_repo)
...
def detect_name_and_version(repo, source, options):
    ...
    try:
        ...
        try:
            ...
            # example).
            (options.packaging_dir, specfile) = guess_spec_repo(repo, options.packaging_branch, options.packaging_dir)
            spec = SpecFile(specfile)
            sourcepackage = spec.name
        except NoSpecError:
...
def parse_args(argv):
    ...
    import_group.add_config_file_option(option_name="pristine-tarball-base",
                      dest="pristine_tarball_base")
...
def main(argv):
    ...
    try:
        ...
        pristine_orig = symlink_orig(source.path, sourcepackage, version, options.pristine_tarball_base)
                ...
                if repo.has_branch(options.packaging_branch):
                    ...
                    if options.postimport:
                        ...
                        try:
                            ...
                            gbp.log.warn("postimport: no spec file found")
                        info = { 'version': "%s%s" % (epoch, version) }
