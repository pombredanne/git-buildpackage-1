...
from gbp.scripts.buildpackage_rpm import (disable_hooks, get_tree,
        ...
        create_packaging_tag, GbpAutoGenerateError)
...
def main(argv):
    ...
    try:
        ...
        if options.tag or options.tag_only:
            ...
            gbp.log.info("Tagging %s" % rpm.compose_version_str(version))
            create_packaging_tag(repo, tree, bbfile.getVar('PN'), version,
                                 options)
