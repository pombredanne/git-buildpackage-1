...
#{ Remote Repositories
    ...
    def fetch(self, repo=None, tags=False, depth=0, refspec=None,
        ...
        @type depth: C{int}
        @param refspec: the format of a <refspec> parameter is an optional plus
    ...
    def pull(self, repo=None, ff_only=False, all_repos=False):
        """
        ...
        @type ff_only: C{bool}
        @param all_repos: fetch all remotes
        @type all_repos: C{bool}
        """
        args = GitArgs()
        args.add_true(ff_only, '--ff-only')
        args.add_true(all_repos, '--all')
...
#{ Patches
    ...
    def diff(self, obj1, obj2=None, paths=None, stat=False, summary=False):
        ...
        @rtype: C{str}
        """
        options = GitArgs('-p', '-a', '--no-ext-diff', '--ignore-submodules')
        if stat is True:
            options.add('--stat')
        elif stat:
